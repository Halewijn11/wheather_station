...
void loop()
{
  switch( deviceState )
  {
    ...
    {
      // 1. Get the newly updated, accurate time
      TimerSysTime_t now = TimerGetSysTime();
      
      // 3. Calculate how many seconds we are into the current 15-minute block
      uint32_t secondsPastMark = now.Seconds % interval;

      // 4. Calculate how long to sleep to hit the NEXT mark exactly
      uint32_t secondsToWait = interval - secondsPastMark;

      // 5. Convert to milliseconds for the cycle function
      txDutyCycleTime = secondsToWait * 1000;

      Serial.printf("Next sync in %u seconds.\r\n", secondsToWait);
