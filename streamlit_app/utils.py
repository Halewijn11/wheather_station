...
def get_moonrise_moonset(latitude=50.924503, longitude=4.112950):
    """
    Moonrise/moonset for today at the given location, formatted as HH:MM.
    The moon doesn't rise or set on every calendar day (its rise/set drifts
    ~50min later each day), so either can be None - returned as "--:--".
    """
    city = LocationInfo(
        name="Affligem",
        region="Belgium",
...
def get_precipitation_forecast(latitude=50.924503, longitude=4.112950, hours=24):
    """
    Hourly precipitation forecast (mm) for the next `hours` hours, from the
    free Open-Meteo API (no key required, reuse explicitly permitted -
    unlike KMI/meteo.be, whose terms forbid republishing their data).
    """
    url = (
        "https://api.open-meteo.com/v1/forecast"
        f"?latitude={latitude}&longitude={longitude}"
        "&hourly=precipitation&timezone=Europe%2FBrussels&forecast_days=2"
    )
    ...
    df = pd.DataFrame({
        'received_at': pd.to_datetime(hourly.get('time', [])),
        'precipitation_mm': hourly.get('precipitation', []),
    })
    df['received_at'] = df['received_at'].dt.tz_localize('Europe/Brussels')

    now_local = pd.Timestamp.now(tz='Europe/Brussels')
    df = df[df['received_at'] >= now_local.floor('h')].head(hours).reset_index(drop=True)
    return df


@st.cache_data(ttl=10 * 60)
def get_rainviewer_radar_frames():
    """
    All available radar frames (host + list of {time, path, is_forecast})
    from RainViewer's free public API: the observed "past" frames (~2h
    back) followed by "nowcast" frames (~30-60min ahead) when RainViewer
    has published a forecast run - same past+forecast animation style as
    Buienradar. Meant for exactly this kind of third-party embedding,
    unlike KMI/meteo.be's radar, whose terms forbid republishing.
    """
    ...
    frames = [{**f, 'is_forecast': False} for f in past_frames] + \
             [{**f, 'is_forecast': True} for f in nowcast_frames]
    return host, frames


def render_radar_map_html(latitude=50.924503, longitude=4.112950, zoom=7, height=400):
    """
    Self-contained Leaflet map (loaded from CDN) with an animated RainViewer
    radar overlay (play/pause button, cycling through ~2h of observed frames
    plus any published forecast/"nowcast" frames - same style as Buienradar)
    on top of OpenStreetMap, centered on the given location.
    Returns None if the radar frames couldn't be fetched.
    """
    frames = get_rainviewer_radar_frames()
    if frames is None:
        return None
    host, all_frames = frames
    tile_templates = [
        f"{host}{f['path']}/256/{{z}}/{{x}}/{{y}}/2/1_1.png" for f in all_frames
    ]
    tz = pytz.timezone('Europe/Brussels')
    frame_labels = [
        datetime.fromtimestamp(f['time'], tz=tz).strftime('%H:%M') + (' (forecast)' if f['is_forecast'] else '')
        for f in all_frames
    ]
    now_index = sum(1 for f in all_frames if not f['is_forecast']) - 1

    return f"""
    <div id="radar-map" style="height:{height}px; border-radius:8px; overflow:hidden;"></div>
    <div style="margin-top:8px; display:flex; align-items:center; gap:10px;">
        <button id="radar-play-btn" style="padding:4px 14px; border-radius:6px; border:1px solid #ccc; background:#fff; cursor:pointer;">&#9654; Play</button>
        <span id="radar-time-label" style="font-family:sans-serif; font-size:14px; color:#333;"></span>
    </div>
    <link rel="stylesheet" href="https://unpkg.com/leaflet@1.9.4/dist/leaflet.css" />
    <script src="https://unpkg.com/leaflet@1.9.4/dist/leaflet.js"></script>
    <script>
        var map = L.map('radar-map').setView([{latitude}, {longitude}], {zoom});
        L.tileLayer('https://{{s}}.tile.openstreetmap.org/{{z}}/{{x}}/{{y}}.png', {{
            attribution: '&copy; OpenStreetMap contributors'
        }}).addTo(map);
        L.marker([{latitude}, {longitude}]).addTo(map);

        var radarTiles = {json.dumps(tile_templates)};
        var radarLabels = {json.dumps(frame_labels)};
        var nowIndex = {now_index};
        var frameIndex = nowIndex;
        var radarLayer = L.tileLayer(radarTiles[frameIndex], {{
            opacity: 0.6,
            maxNativeZoom: 7,
            attribution: 'Radar &copy; RainViewer'
        }}).addTo(map);
        document.getElementById('radar-time-label').textContent = radarLabels[frameIndex];
        ...
        function showFrame(i) {{
            frameIndex = i;
            radarLayer.setUrl(radarTiles[frameIndex]);
            document.getElementById('radar-time-label').textContent = radarLabels[frameIndex];
        }}

        function tick() {{
            showFrame((frameIndex + 1) % radarTiles.length);
        }}
...
def get_moonphase_filepath(image_repo = './'):
    ...
    return filepath, phase_index, illumination_pct
